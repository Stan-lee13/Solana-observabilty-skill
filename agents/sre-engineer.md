...
|----------|------------|----------|--------------|
| P0 | User funds at risk OR >5% TX failure OR total outage | Immediate — wake anyone | PagerDuty + phone call |
| P1 | Degraded service, >2% TX failure, RPC issues | <15 min response | PagerDuty |
  ...
  - name: solana-dapp-critical
    ...
    rules:
      ...
      - alert: TransactionFailureRateHigh
        expr: |
          (
            sum(rate(solana_transaction_failed_total[5m]))
            /
        ...
        labels:
          ...
          team: protocol
        annotations:
          summary: "Transaction failure rate {{ $value | humanizePercentage }} (threshold: 5%)"
