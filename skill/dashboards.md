  ...
  "dashboard": {
    ...
    "panels": [
      ...
      },
      {
        ...
        "type": "timeseries",
        "targets": [
          { "expr": "histogram_quantile(0.50, rate(solana_rpc_latency_ms_bucket[5m]))", "legendFormat": "P50" },
          { "expr": "histogram_quantile(0.95, rate(solana_rpc_latency_ms_bucket[5m]))", "legendFormat": "P95" },
          { "expr": "histogram_quantile(0.99, rate(solana_rpc_latency_ms_bucket[5m]))", "legendFormat": "P99" }
        ],
        "fieldConfig": {
          "defaults": { "unit": "ms" }
        },
      ...
      - record: solana:rpc_health_score
        expr: |
          (
            ...
            (1 - clamp(solana_slot_lag / 100, 0, 1)) * 0.3 +
            (1 - clamp(solana_rpc_latency_ms / 5000, 0, 1)) * 0.3
          )
